...
f1 = open("./OAG-v2-track1/train_pub.json", "r", encoding="utf-8")
pubs_raw = json.load(f1)
f2 = open("./OAG-v2-track1/train_author.json", "r", encoding="utf-8")
...
for length in range(150, 210, 20):
    result = []
    print("len", length)
    for n, name in enumerate(name_pubs):
        ...
        for k in range(rw_num):
            ...
            model = word2vec.Word2Vec(sentences, size=length, min_count=1, iter=10, negative=25, window=10)
            embs = []
            for i, pid in enumerate(pubs):
                if pid in model:
        ...
        # evaluate
        ###############################################################
        pre = DBSCAN(eps=eps, min_samples=4, metric="precomputed").fit_predict(sim)
